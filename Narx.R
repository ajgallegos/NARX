...
CopyNode <- R6Class(classname = "CopyNode", inherit = Node, public = list(
  ...
  load_source_value = function(){
    ...
    copy$value_ <- copy$value_ * self$existing_weight_ + value * self$incoming_weight_
  },
  ...
  source_update_config = function(source_type, incoming_weight, existing_weight){
    ...
    errormsg <- "The incoming weight must be numeric between 0 and 1."
    ...
    errormsg <- "The existing weight must be numeric between 0 and 1."
